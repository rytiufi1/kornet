...
public class RobloxPlayerCorsMiddleware
{
    ...
    private static readonly Regex AllowedOriginPattern = new(@"^https://(([a-z0-9-]+\.)*)?kornet\.lat$", RegexOptions.Compiled);
    ...
    public async Task InvokeAsync(HttpContext ctx)
    {
        ...
        if (!string.IsNullOrEmpty(origin) && AllowedOriginPattern.IsMatch(origin))
        {
        ...
        var isAuthenticated = ctx.Request.Cookies.ContainsKey(".ROBLOSECURITY");
