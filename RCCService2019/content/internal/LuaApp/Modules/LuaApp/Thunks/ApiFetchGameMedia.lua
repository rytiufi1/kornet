...
local ApiFetchGameMedia = {}

local keyMapper = function(universeId)
	return "luaapp.gamesapi.gamemedia."..universeId
end

ApiFetchGameMedia.KeyMapper = keyMapper

function ApiFetchGameMedia.Fetch(networkImpl, universeId)
	assert(typeof(universeId) == "string")
	assert(#universeId > 0)

	local fetchKey = keyMapper(universeId)
...
function ApiFetchGameMedia.GetFetchingStatus(state, universeId)
	return PerformFetch.GetStatus(state, keyMapper(universeId))
end

return ApiFetchGameMedia