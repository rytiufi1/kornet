...
local function loginByEmail(networkImpl, cvalue, password)
	return function(store)
		return LoginRequests.logByEmail(networkImpl, cvalue, password):andThen(
			function(result)
			...
			function(err)
				...
				if body.errors[1].code == LoginErrorCodes.IncorrectCValueOrPassword then
					return Promise.reject("Authentication.Login.Response.IncorrectEmailOrPassword")
				...
				elseif body.errors[1].code == LoginErrorCodes.MoreThanOneUsername then
					return Promise.reject("Authentication.Login.Response.MoreThanOneUsername")
				elseif body.errors[1].code == LoginErrorCodes.ServiceUnavailable then
					...
					return Promise.reject("Authentication.Login.Response.ErrorLoginWithUsername")
				end
			end
		)
	end
