...
-- The purpose of this thunk is to ONLY navigate back if the current view is a native view wrapper.
--
...
return function()
	return function(store)
		...
		if not currentPage.nativeWrapper then
			return
		end

		if FFlagLuaNavigationLockRefactor then
			-- bypass navigation lock since we're coming from native
			store:dispatch(ApplyNavigateBack(true))
		else
