	...
	return function(store)
		spawn(function()
			...
			local assetsWebCall, status = AEWebApi.GetUserInventory(assetType,
				ASSET_CARDS_PER_PAGE, userId, nextCursor)
			...
			if assetsWebCall.Data.Items then
				...
				for _, asset in pairs(assetsWebCall.Data.Items) do
					local assetId = tostring(asset.Item.AssetId)
					ownedAssets[#ownedAssets + 1] = assetId
					parsedAssetInfo[assetId] = state.AEAppReducer.AEAssetInfo[assetId] and
						state.AEAppReducer.AEAssetInfo[assetId] or AEAssetModel.fromWebApi(asset, assetType)
				end
