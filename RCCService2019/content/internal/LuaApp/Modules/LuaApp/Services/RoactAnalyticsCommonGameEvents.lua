...
local quickGameLaunch = require(Modules.LuaApp.Analytics.Events.quickGameLaunch)
...
function RoactAnalyticsCommonGameEvents.get(context, args)
	...
	local function reportGameDetailReferral(referralPage,
		...
		local referralContext = createReferralCtx(indexInSort, sortId, gameSetTargetId, timeFilter, genreFilter)
		gameDetailReferral(analyticsImpl.EventStream, referralContext, referralPage, numItemsInSort, placeId, isAd)
	end

	function CGE.reportOpenGameDetail(placeId, sortName, gameSetTargetId, indexInSort, itemsInSort, isAd, timeFilter, genreFilter)
		reportGameDetailReferral(pageName, placeId, sortName, gameSetTargetId, indexInSort, itemsInSort, isAd, timeFilter, genreFilter)
	end

	function CGE.reportOpenGameDetailFromSeeAll(placeId, sortName, gameSetTargetId, indexInSort, itemsInSort, isAd, timeFilter, genreFilter)
		reportGameDetailReferral(pageNameSeeAll, placeId, sortName, gameSetTargetId, indexInSort, itemsInSort, isAd, timeFilter, genreFilter)
	end

	function CGE.reportQuickGameLaunchEntry()
		quickGameLaunch(analyticsImpl.EventStream, pageName, "EnterQuickLaunchFlow")
	end

	function CGE.reportQuickGameLaunchSuccess()
		quickGameLaunch(analyticsImpl.EventStream, pageName, "QuickLaunchSucceed")
	end

	function CGE.reportQuickGameLaunchFailed(reason)
		quickGameLaunch(analyticsImpl.EventStream, pageName, "QuickLaunchFailed", reason)
	end
	...
	function CGE.reportTouchSearchInsteadForPlayerNamed(name)
		buttonClick(analyticsImpl.EventStream, pageName, "searchInsteadForPlayerNamed", name)
	end
