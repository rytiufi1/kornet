...
return function(state, action)
	...
	if action.type == AEAddAssetsInfo.name then
		local newAssets = {}
		for key, value in pairs(action.assets) do
			newAssets[tostring(key)] = value
		end

		return Immutable.JoinDictionaries(state, newAssets)
	elseif action.type == AEReceivedAvatarData.name then
		...
		for _, asset in ipairs(actionAssets) do
			...
			assets[tostring(asset.id)] = assetInfo
		end
