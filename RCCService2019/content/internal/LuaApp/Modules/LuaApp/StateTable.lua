...
--[[
	It is recommended that you use StateTableBuilder to construct a new StateTable instance
	instead of calling StateTable.new() directly. That being said, the functionality of
	StateTable is explained here, so you should also read the rest of this guide.
	...
	logic that is based upon your state machine design. Ex:
		...
		self.stateTable = StateTable.new(loggingName, initialState, initialContext, {
			InitialState = {
	...
	Arguments:
		loggingName 	- A tag to use for logging state transitions if loggingEnabled == true. (String)
...
function StateTable:handleEvent(eventKey, args)
	...
	if eventMap[eventKey] ~= nil then
		...
		if self.loggingEnabled then
			print(string.format("%s: changing state, '%s' [%s] --> '%s'",
				self.name, currentState, eventKey, nextState))
		end

		local updatedContext = self.currentContext
		if action ~= nil then
			local contextDiff = action(currentState, nextState, args, self.currentContext) or {}
			updatedContext = Immutable.JoinDictionaries(self.currentContext, contextDiff)
		...
		if self.loggingEnabled then
			Logging.warn(string.format("%s: unhandled state transition (%s[%s])",
				self.name, currentState, eventKey))
		end
	end
end
