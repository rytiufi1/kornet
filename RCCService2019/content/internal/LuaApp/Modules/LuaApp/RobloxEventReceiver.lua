...
function RobloxEventReceiver.new(notificationService)
	...
	self.connections = {
		...
		notificationService.RobloxConnectionChanged:Connect(function(connectionName, connectionState, sequenceNumber)
			-- Check and make sure that someone is observing this namespace
			local signal = self.namespaceTable[connectionName]
			if signal == nil then
				return
			end

			signal:fire(connectionState, sequenceNumber)
		end)
	}
