...
MutedError
	...
	Otherwise reports the error to GoogleAnalytics.
...
return function(errorMessage)
	if type(errorMessage) ~= "string" then
		...
		if not success then
			...
			-- At least we could still use the call stack information in GA.
	...
	if IsRunningInStudio() or _G.__TESTEZ_RUNNING_TEST__ then
		...
		LuaAppEvents.ReportMutedError:fire(errorMessage, debug.traceback(), script)
	end
end
