return function()
	...
	local GenericWebPageWrapper = require(script.parent.GenericWebPageWrapper)

	local TEST_URL = "the_test_url"
	local TEST_TITLE = "the_test_title"

	describe("GenericWebPageWrapper", function()
		it("should broadcast the OPEN_CUSTOM_WEBVIEW notification when mounted", function()
			local guiService = MockGuiService.new()
			local element = mockServices({
				wrapper = Roact.createElement(GenericWebPageWrapper, {
					isVisible = true,
					url = TEST_URL,
					title = TEST_TITLE,
				}),
			...
			expect(guiService.broadcasts[1].notification).to.equal(NotificationType.OPEN_CUSTOM_WEBVIEW)
