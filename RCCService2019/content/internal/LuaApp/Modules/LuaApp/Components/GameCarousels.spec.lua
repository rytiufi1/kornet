return function()
	...
	it("should create and destroy without errors with one carousel", function()
		...
		local store = Rodux.Store.new(AppReducer, {
			...
			RequestsStatus = {
				GameSortsStatus = {
					[gameSort.name] = RetrievalStatus.Done,
				}
			},
		})
	...
	it("should create and destroy without errors with more than one carousel", function()
		...
		local gameSortContents2 = GameSortContents.new()
		...
		gameSortContents1.entries = { entry1, entry2 }
		...
		local store = Rodux.Store.new(AppReducer, {
			...
			RequestsStatus = {
				GameSortsStatus = {
					[gameSort1.name] = RetrievalStatus.Done,
					[gameSort2.name] = RetrievalStatus.Failed,
				}
			}
		})
		local element = mockServices({
