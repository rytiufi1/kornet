...
function AEWebApi.SetEmotes(emotesInfo)
	local data = jsonEncode(emotesInfo)
	local url =  AVATAR_URL .. "/v1/emotes/set-all-emotes"
...
function AEWebApi.GetUserCostumes(userId, desiredPageNumber, itemsPerPage, isEditable)
	local query = makeQueryString({
		page = desiredPageNumber,
		itemsPerPage = itemsPerPage,
