...
local FFlagAvatarEditorRemoveDefaultClothingMessage = settings():GetFFlag("AvatarEditorRemoveDefaultClothingMessage")
...
function AEBodyColorManager.new(store, characterManager)
	...
	local userId = FFlagAvatarEditorUseUserIdFromStore and store:getState().LocalUserId or game.Players.LocalPlayer.userId
	local defaultClothesIndex = tonumber(userId)
...
function AEBodyColorManager:manageDefaultClothing(curEquipped)
	...
	local putOnDefaultClothing, tookOffDefaultClothing = false, false -- FFlagAvatarEditorRemoveDefaultClothingMessage - Remove these 2 variables when flag is removed
	...
	if not FFlagAvatarEditorRemoveDefaultClothingMessage then
