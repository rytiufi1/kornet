...
function AEUtils.isPresetCostume(costume)
    return not costume.isEditable
end

function AEUtils.gamepadNavigationEnabled()
    return IS_CONSOLE
end
...
function AEUtils.getCurrentCategory(categoryIndex)
    return AECategories.categories[categoryIndex]
end
...
function AEUtils.getEquippedAssetIds(equippedAssets)
    ...
    if FFAvatarEditorEquippedAssetsNilIndexingFix and not equippedAssets then
        return assetIds
    end
