...
local FFlagAvatarPartScaleTypeFix = settings():GetFFlag("AvatarPartScaleTypeFix")
local FFlagAvatarEditorFixToolStuckOnRightArmEquip = settings():GetFFlag("FFlagAvatarEditorFixToolStuckOnRightArmEquip")
...
function AECharacterManager:update(newState, oldState)
	...
	if FFlagAvatarEditorEmotesSupport then
		local curEmotes = newState.AEAppReducer.AEEquippedEmotes.slotInfo
		local oldEmotes = oldState.AEAppReducer.AEEquippedEmotes.slotInfo
		...
		if curEmotes ~= oldEmotes then
			for slot, emoteInfo in pairs(curEmotes) do
				...
				if not oldEmoteInfo or oldEmoteInfo.assetId ~= emoteInfo.assetId then
					equippedIdsWithAssetType[AEConstants.AssetTypes.Emote] = { emoteInfo.assetId }
				end
			end

			for slot, emoteInfo in pairs(oldEmotes) do
				if not curEmotes[slot] then
					unequippedIdsWithAssetType[AEConstants.AssetTypes.Emote] = { emoteInfo.assetId }
				end
			end
		end
	end
	...
	self.animationManager:checkForUpdate(equippedIdsWithAssetType, unequippedIdsWithAssetType, newState, oldState)
end
...
function AECharacterManager:equipAsset(assetId, assetTypeId, assetModel)
	...
	for _, modelChild in pairs(modelChildren) do
		...
		elseif isAccessory(modelChild) then
			...
			self.store:dispatch(AEAccessoryChangeOnModel())
		...
		elseif assetTypeId == AEConstants.AssetTypes.TShirt then
...
function AECharacterManager:unequipAsset(assetId, assetTypeId, isPotentialSwap)
	...
	if not isPotentialSwap then
		...
		if assetTypeId == AEConstants.AssetTypes.Head then
			...
			if FFlagAvatarPartScaleTypeFix then
				self:buildRig()
			end
		elseif assetTypeId == AEConstants.AssetTypes.Face then
		...
		elseif assetTypeId == AEConstants.AssetTypes.Pants and self.pants then
			...
			self.pants:Destroy()
			self.pants = nil
		elseif assetTypeId == AEConstants.AssetTypes.TShirt
...
function AECharacterManager:switchAvatarType(newType)
	...
	-- Apply the original size head mesh to an R6.
	if newType == AEConstants.AvatarType.R6 then
		local scale = self.head:FindFirstChild("OriginalSize")
		if scale then
			self.head.Scale = Vector3.new(scale.Value.X, scale.Value.Y, scale.Value.Z)
		end
	end
	...
	if not FFlagAvatarEditorFixToolStuckOnRightArmEquip then
...
function AECharacterManager:buildRig()
	if self.currentCharacter == self.r15 then
		...
		self.scalesManager:adjustHeightToStandOnPlatform(self.currentCharacter)
	end
	if FFlagAvatarEditorFixToolStuckOnRightArmEquip then
		for _, tool in pairs(self.equippedToolModels) do
			self:equipTool(tool)
		end
	end
