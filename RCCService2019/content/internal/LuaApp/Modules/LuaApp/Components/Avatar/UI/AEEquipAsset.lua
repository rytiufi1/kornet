...
function AEEquipAsset:init()
	...
	self.checkIfWearingAsset = function(assetId, isOutfit, override)
		...
		if categoryPage.name == AEConstants.EMOTES then
			local focusedIndex = self.props.tabsInfo[self.props.categoryIndex]
			local emoteInfo = self.props.emotesSlotInfo[focusedIndex]

			if emoteInfo then
				return emoteInfo.assetId == assetId
			end
...
function AEEquipAsset:shouldUpdate(nextProps, nextState)
	...
	if self.props.emotesSlotInfo ~= nextProps.emotesSlotInfo then
		return true
	end

	local oldCategoryPage = AEUtils.getCurrentCategory(self.props.categoryIndex)
	local categoryPage = AEUtils.getCurrentCategory(nextProps.categoryIndex)

	if oldCategoryPage ~= categoryPage then
		return true
	end

	if categoryPage.name == AEConstants.EMOTES then
		if self.props.tabsInfo ~= nextProps.tabsInfo then
			return true
		end
	end
...
AEEquipAsset = RoactRodux.UNSTABLE_connect2(
	function(state, props)
		return {
			...
			emotesSlotInfo = state.AEAppReducer.AEEquippedEmotes.slotInfo,
