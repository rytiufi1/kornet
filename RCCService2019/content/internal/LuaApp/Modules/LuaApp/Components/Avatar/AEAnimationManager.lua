...
local FFlagAvatarEditorFixLookAroundJumpiness = settings():GetFFlag("AvatarEditorFixLookAroundJumpiness2")
...
function AEAnimationManager:start()
	...
	spawn(function()
		...
		elseif currentCategoryIndex == EMOTES_PAGE then
			self:playAnimation(self:getEquippedEmoteOrDefaultAnim(currentTab), true)
		else
...
function AEAnimationManager:playLookAround(animationAssets, id)
	if not FFlagAvatarEditorFixLookAroundJumpiness then
	...
	if assetId and assetId[1] then
		...
		if not assets then
			if FFlagAvatarEditorFixLookAroundJumpiness then
...
function AEAnimationManager:getWeightedAnimations(possible)
	...
	for _, animation in next, possible do
		...
		if not FFlagAvatarEditorFixLookAroundJumpiness or weight > 0 then
...
function AEAnimationManager:getDefaultAnimationAssets(assetTypeId, currentCharacter)
	...
	elseif assetTypeId == AEConstants.AssetTypes.Emote then
		table.insert(anims, currentCharacter.Animations.idle)
...
function AEAnimationManager:getEquippedEmoteOrDefaultAnim(tabIndex)
	local currentCharacter = self.store:getState().AEAppReducer.AECharacter.AECurrentCharacter
	local slotInfo = self.store:getState().AEAppReducer.AEEquippedEmotes.slotInfo

	local emoteInfo = slotInfo[tabIndex]
	if emoteInfo then
		return self:loadAnimationObjects(emoteInfo.assetId)
	end

	return {
		currentCharacter.Animations.idle
	}
end
...
function AEAnimationManager:getAnimationFromCategory(currentCategoryIndex, currentTab)
	if currentCategoryIndex == EMOTES_PAGE then
		return self:getEquippedEmoteOrDefaultAnim(currentTab)
	else
		return self:getEquippedOrDefaultAnim(animTabIndexToAnimType[currentTab], currentCategoryIndex)
	end
end

function AEAnimationManager:checkForUpdate(equippedIds, unequippedIds, newState, oldState)
	...
	local canPlayAnimation = (currentCategoryIndex == ANIMATION_PAGE or currentCategoryIndex == RECENTS_PAGE
		or currentCategoryIndex == EMOTES_PAGE) and avatarType == AEConstants.AvatarType.R15
	...
	spawn(function()
		...
		elseif equippedItem or rebuiltCharacter then
			...
			local assets = self:getAnimationFromCategory(currentCategoryIndex, currentTab)
			self.indexOffset = false
			...
			elseif FFlagAvatarEditorFixLookAroundJumpiness then
				self:playAnimation(assets, true)
			end
		elseif changedTab or unequippedAnimation then
			self.indexOffset = false
			self:playAnimation(self:getAnimationFromCategory(currentCategoryIndex, currentTab), true)
