...
local MEMBERSHIP_LOGOS = {
	[Enum.MembershipType.BuildersClub] = "LuaApp/icons/ic-bc",
	[Enum.MembershipType.TurboBuildersClub] = "LuaApp/icons/ic-tbc",
	[Enum.MembershipType.OutrageousBuildersClub] = "LuaApp/icons/ic-obc",
	[Enum.MembershipType.Premium] =
		FFlagLuaAppUseNewPremiumIcon() and "LuaApp/icons/status_premium" or "LuaApp/icons/ic-bc",
}

local BUILDERCLUB_LOGO_WIDTH_COMPACT = 24
local BUILDERCLUB_LOGO_HEIGHT_COMPACT = 24
local MEMBERSHIP_LOGOS_COMPACT = {
	[Enum.MembershipType.BuildersClub] = "LuaApp/icons/ic-bc-small",
	[Enum.MembershipType.TurboBuildersClub] = "LuaApp/icons/ic-tbc-small",
	[Enum.MembershipType.OutrageousBuildersClub] = "LuaApp/icons/ic-obc-small",
	[Enum.MembershipType.Premium] =
		FFlagLuaAppUseNewPremiumIcon() and "LuaApp/icons/status_premium" or "LuaApp/icons/ic-bc",
}

local PREMIUM_LOGO_SIZE = 24
...
local PROFILE_PICTURE_THUMBNAIL_TYPE

if FFlagLuaAppMakeAvatarThumbnailTypesEnum then
	PROFILE_PICTURE_THUMBNAIL_TYPE = AvatarThumbnailTypes.HeadShot
else
	PROFILE_PICTURE_THUMBNAIL_TYPE = Constants.AvatarThumbnailTypes.HeadShot
end
...
local function createNormalHeaderInfo(props)
	...
	local premiumIconStyle = props.premiumIconStyle

	local playerHasMembership = membership ~= Enum.MembershipType.None
	local isPlayerPremium = membership == Enum.MembershipType.Premium
	local membershipIconSize = isPlayerPremium and UDim2.new(0, PREMIUM_LOGO_SIZE, 0, PREMIUM_LOGO_SIZE) or
		UDim2.new(0, BUILDERCLUB_LOGO_WIDTH, 0, BUILDERCLUB_LOGO_HEIGHT)
	...
	return Roact.createElement(FitChildren.FitFrame, {
		...
		BuildersClubUsernameFrame = Roact.createElement("Frame", {
			...
			Membership = playerHasMembership and Roact.createElement(ImageSetLabel, {
				Size = membershipIconSize,
				Image = MEMBERSHIP_LOGOS[membership],
				ImageColor3 = isPlayerPremium and premiumIconStyle.Color or nil,
				ImageTransparency = isPlayerPremium and premiumIconStyle.Transparency or nil,
				BackgroundTransparency = 1,
				BorderSizePixel = 0,
				LayoutOrder = 2,
			}),
...
-- Mobile phone(compact view) is a special case.
local function createPhoneHeaderInfo(props)
	...
	local isPlayerPremium = membership == Enum.MembershipType.Premium
	local membershipIconSize = isPlayerPremium and UDim2.new(0, PREMIUM_LOGO_SIZE, 0, PREMIUM_LOGO_SIZE) or
		UDim2.new(0, BUILDERCLUB_LOGO_WIDTH_COMPACT, 0, BUILDERCLUB_LOGO_HEIGHT_COMPACT)
	...
	return Roact.createElement("Frame", {
			...
			Layout = Roact.createElement("UIListLayout", {
				...
				FillDirection = Enum.FillDirection.Horizontal,
				VerticalAlignment = Enum.VerticalAlignment.Center,
				Padding = UDim.new(0, playerHasMembership and USERNAME_BC_PADDING or 0),
			}),
			Padding = Roact.createElement("UIPadding", {
				PaddingLeft = UDim.new(0, sidePadding),
			}),
			Membership = playerHasMembership and Roact.createElement(ImageSetLabel, {
				Size = membershipIconSize,
				Image = MEMBERSHIP_LOGOS_COMPACT[membership],
				ImageColor3 = isPlayerPremium and premiumIconStyle.Color or nil,
				ImageTransparency = isPlayerPremium and premiumIconStyle.Transparency or nil,
				BackgroundTransparency = 1,
				LayoutOrder = 1,
...
function HomeHeaderUserInfo:init()
	self.onUsernameActivated = function()
		...
		if FFlagLuaAppHttpsWebViews then
			...
			url = Url:getUserProfileUrl(localUserId)
		end
...
function HomeHeaderUserInfo:render()
	...
	local isCompact = formFactor == FormFactor.COMPACT

	if UseNewAppStyle then
		return withStyle(function(style)
			return (isCompact and createPhoneHeaderInfo or createNormalHeaderInfo)({
				localUserModel = localUserModel,
				sidePadding = sidePadding,
				sectionPadding = sectionPadding,
				onUsernameActivated = self.onUsernameActivated,
				premiumIconStyle = style.Theme.TextEmphasis,
			})
		end)
	else
		return (isCompact and createPhoneHeaderInfo or createNormalHeaderInfo)({
			...
			sectionPadding = sectionPadding,
			onUsernameActivated = self.onUsernameActivated,
			premiumIconStyle = theme.PremiumIcon,
		})
	end
end
