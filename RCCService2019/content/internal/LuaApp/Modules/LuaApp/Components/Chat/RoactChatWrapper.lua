...
function RoactChatWrapper:updateChat()
	...
	chatMaster:SetAppPolicy(appPolicy)
...
RoactChatWrapper = RoactServices.connect({
	analytics = RoactAnalyticsAppStageLoaded,
	guiService = AppGuiService,
})(RoactChatWrapper)

if FFlagLuaAppPolicyRoactConnector then
	RoactChatWrapper = RoactAppPolicy.connect(function(appPolicy, props)
		return {
			appPolicy = appPolicy,
		}
	end)(RoactChatWrapper)
else
	RoactChatWrapper = RoactAppPolicy.legacy_connect(function(appPolicy, props)
		return {
			appPolicy = appPolicy,
		}
	end)(RoactChatWrapper)
