...
local WHITE_ARROW_PATH = "LuaApp/icons/icon_retry_white"
...
function RefreshScrollingFrame:init()
	...
	self.onCanvasPositionChanged = function(rbx)
		...
		local onLoadMore = self.props.onLoadMore
		local isLoadingMore = self.state.isLoadingMore
		local onCanvasPositionChangedCallback = self.props.onCanvasPositionChangedCallback
		...
		if FFlagLuaAppRefreshScrollingFrameLoadMoreRefactor then
			-- There exists a callback
			if onCanvasPositionChangedCallback then
				onCanvasPositionChangedCallback(rbx)
			end
		else
...
function RefreshScrollingFrame:render()
	...
	local function renderRSF(rootBackgroundColor3, rootBackgroundTransparency, spinnerColor)
		return Roact.createElement("Frame", {
			Size = UDim2.new(1, 0, 1, 0),
			BackgroundColor3 = rootBackgroundColor3,
			BackgroundTransparency = rootBackgroundTransparency,
		}, {
			...
			scrollingFrame = Roact.createElement(FitChildren.FitScrollingFrame, {
				...
				Footer = (FFlagLuaAppRefreshScrollingFrameLoadMoreRefactor and createFooter) and
				...
				LoadingBarFrame =
				(not FFlagLuaAppRefreshScrollingFrameLoadMoreRefactor and isLoadingMore) and Roact.createElement("Frame", {
					BackgroundTransparency = 1,
					LayoutOrder = 2,
	...
	if UseNewAppStyle then
		return withStyle(function(style)
			return renderRSF(style.Theme.BackgroundDefault.Color,
				style.Theme.BackgroundDefault.Transparency,
				style.Theme.SystemPrimaryDefault.Color)
		end)
	else
		return renderRSF(backgroundColor3, backgroundTransparency, nil)
	end
end
