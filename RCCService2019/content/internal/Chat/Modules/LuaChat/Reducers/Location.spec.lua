return function()
	...
	describe("SetRoute with popToIntent", function()
		it("should pop to the right intent first and then add the new intent on top", function()
			local state = Location(nil, {})
			local startingHistoryLength = #state.history

			state = Location(nil, SetRoute(Intent.ConversationHub, {}))
			...
			expect(#state.history).to.equal(startingHistoryLength + 4)
			...
			expect(#state.history).to.equal(startingHistoryLength + 2)
		...
		it("should pop to the right intent and stop if the new intent is nil", function()
			...
			expect(#state.history).to.equal(startingHistoryLength + 3)
	...
	describe("route history", function()
		it("should push history", function()
			...
			expect(#state.history).to.equal(startingHistoryLength + 1)
		end)
		...
		it("should have current be equal to top of history", function()
			...
			expect(state.history[#state.history].intent).to.equal(state.current.intent)
		end)
		...
		if FFlagLuaChatScreenManagerAlwaysUpdatesWithDefaults then
			it("should never pop routes where the history is empty", function()
				...
				expect(state.current.intent).to.equal(Intent.ConversationHub)
				expect(#state.history).to.equal(startingHistoryLength)
			...
			it("should never pop routes where the history is empty", function()
				...
				state = Location(state, SetRoute(Intent.ConversationHub))
	...
	if FFlagLuaChatScreenManagerAlwaysUpdatesWithDefaults then
		describe("Navigating to Conversation and popping route", function()
			it("SHOULD set current Location to ConversationHub", function()
				local state = Location(nil, SetRoute(
					Intent.Conversation,
					{
						conversationId = "testConvoId"
	...
	if FFlagLuaChatContactSettingsOffConversationFix then
		describe("Setting Privacy Settings to Off", function ()
			it("should clear any existing conversation screen", function ()
				local state = Location(nil, FetchChatSettingsCompleted(
					{
						settings = {
							chatEnabled = false
						}
