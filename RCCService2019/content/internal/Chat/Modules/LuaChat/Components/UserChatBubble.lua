...
local UserChatBubbleModerationError = require(LuaChat.Components.UserChatBubbleModerationError)
local FlagSettings = require(LuaChat.FlagSettings)
local LuaAppsFlagSettings = require(LuaApp.FlagSettings)
local Message = require(LuaChat.Models.Message)

local FFlagLuaChatResizeUserChatBubbleModerationError =
	settings():GetFFlag("LuaChatResizeUserChatBubbleModerationError")

local BUBBLE_PADDING = 10
local TEXT_BUBBLE_X_PADDING = 54
local TEXT_BUBBLE_X_WIDE_ADDITIONAL_PADDING = 112
local USER_CHAT_BUBBLE_MODERATION_ERROR_LAYOUT_ORDER = 3
...
function UserChatBubble.new(appState, message, newContent, width)
	...
	if LuaAppsFlagSettings.UseNewAppStyle() then
		if isOutgoingMessage(message) then
			local bubbleColor = Constants.Color.GRAY_PRIMARY
			local bubbleTransparency = isSending and Constants.Color.ALPHA_PENDING_CHAT
			...
			local bubbleColor = Constants.Color.BLACK
			local bubbleTransparency = Constants.Color.ALPHA_SECONDARY
...
function UserChatBubble:Resize()
	...
	if self.appState.store:getState().FormFactor == FormFactor.WIDE then
		padding = padding + TEXT_BUBBLE_X_WIDE_ADDITIONAL_PADDING
	end
	...
	if FFlagLuaChatResizeUserChatBubbleModerationError and self.moderatedText then
		self.moderatedText:ResizeToWidth(viewportWidth)
	end
...
function UserChatBubble:Update(message)
	...
	if filteringTextKey and (not self.moderatedText) then
		if FFlagLuaChatResizeUserChatBubbleModerationError then
			self.moderatedText = UserChatBubbleModerationError.new(self.appState, filteringTextKey)
			self.moderatedText:setLayoutOrder(USER_CHAT_BUBBLE_MODERATION_ERROR_LAYOUT_ORDER)
			self.moderatedText:setTextColor(textColor)
			self.moderatedText:setParent(self.rbx)
		else
