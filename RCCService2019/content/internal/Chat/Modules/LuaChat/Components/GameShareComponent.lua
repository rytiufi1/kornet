...
local SendGameLinkMessage = require(LuaChat.Thunks.SendGameLinkMessage)
...
local ChatFlagSettings = require(LuaChat.FlagSettings)
...
function GameShareComponent:Start()
	...
	-- search clear button
	local clearButtonConnection = self.clearSearchButton.Activated:Connect(function()
		self.searchBox.Text = ""
	end)
...
function GameShareComponent:FillConversations()
	...
	local tappedConnection = list.ConversationTapped:connect(function(convoId)
		self.appState.store:dispatch(SendGameLinkMessage(
			convoId,
			self.placeInfo.universeId
		))
		:andThen(function(response)
			self:ReportSendButtonTappedEvent(convoId, response.responseCode)
		end)
	end)
