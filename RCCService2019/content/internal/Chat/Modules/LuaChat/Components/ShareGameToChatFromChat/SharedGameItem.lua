...
local ShareGame = require(Modules.LuaChat.Thunks.ShareGameToChatFromChat.ShareGame)
local FlagSettings = require(Modules.LuaChat.FlagSettings)
...
local openGameDetailsPage = require(Modules.LuaChat.Thunks.openGameDetailsPage)
...
function SharedGameItem:init()
	...
	self.onGameButtonActivated = function()
		if FFlagLuaChatNavigateToLuaGameDetails then
			local universeId = self.props.game and self.props.game.universeId
			self.props.openGameDetailsPage(universeId)
		else
			-- TODO: SOC-3805 Can remove string coercion when using proper model
			local placeId = self.props.game and tostring(self.props.game.placeId)
			if placeId then
				local notificationType = GuiService:GetNotificationTypeList().VIEW_GAME_DETAILS_ANIMATED
				GuiService:BroadcastNotification(string.format("%s", placeId), notificationType)
			end
		end
	end
	...
	self.onSendButtonActivated = function()
		...
		if universeId then
			if not isSharing then
				self.props.shareGameToChat(activeConversationId, analytics, universeId)
			end
		end
	end
end
...
function SharedGameItem:render()
	...
	local gameId = gameModel and tostring(gameModel.placeId)
	...
	function(dispatch)
		return {
			shareGameToChat = function(activeConversationId, analytics, universeId)
				...
				return dispatch(ShareGame(activeConversationId, universeId))
			end,

			openGameDetailsPage = function(universeId)
				dispatch(openGameDetailsPage(universeId))
			end,
		}
	end
)(SharedGameItem)
