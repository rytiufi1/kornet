return function()
	...
	if FFlagLuaChatGameLinkRender then
		describe("MessageTypes", function()
			it("SHOULD create AssetCard when passed Link MessageType", function()
				local appState = AppState.mock()
				local message = MessageModel.mock({
					gameLink = {
						universeId = "000",
			...
			it("SHOULD create ChatBubble when passed PlainText MessageType", function()
				local appState = AppState.mock()
				local message = MessageModel.mock({
					content = "hi",
	...
	describe("PlainText AssetCard creation", function()
		describe("web protocol filtering", function()
			it("should make a card with http:// prefix", function()
				...
				expect(#chat.bubbles).to.equal(1)
				expect(chat.bubbles[1].bubbleType).to.equal("AssetCard")
				expect(chat.bubbles[1].placeId).to.equal("1818")
			end)
		...
		describe("web resource names", function()
			describe("www.", function()
				it("should make a card with https:// protocol", function()
					...
					expect(chat1.bubbles[1].placeId).to.equal("1818")
				end)
				it("should make a card with http:// protocol", function()
					...
					expect(chat2.bubbles[1].placeId).to.equal("1818")
				end)
				...
				it("should make a card with no protocol", function()
					...
					expect(chat3.bubbles[1].placeId).to.equal("1818")
				end)
			end)
		...
		describe("whitelisted domains", function()
			...
			if FFlagChinaLicensingApp then
				describe("www.roblox.qq.com", function()
					it("should make a card with http protocol prefixes", function()
						local appState = AppState.mock()
						local message = MessageModel.mock({
							content = "https://www.roblox.qq.com/games/1818/Classic-Crossroads"
						})
					...
					it("should make a card with only a resouce name", function()
						local appState = AppState.mock()
						local message = MessageModel.mock({
							content = "www.roblox.qq.com/games/1818/Classic-Crossroads"
						})
				...
				describe("www.roblox.cn", function()
					it("should make a card with http protocol prefixes", function()
						local appState = AppState.mock()
						local message = MessageModel.mock({
							content = "https://www.roblox.cn/games/1818/Classic-Crossroads"
						})
					...
					it("should make a card with only a resouce name", function()
						local appState = AppState.mock()
						local message = MessageModel.mock({
							content = "www.roblox.cn/games/1818/Classic-Crossroads"
						})
		...
		describe("plain text in game link format", function()
		...
		describe("roblox links with text", function()
			...
			it("should create one UserChatBubble with placeholder text if unknown message type is used", function()
				...
				expect(#chat.bubbles).to.equal(1)
				expect(chat.bubbles[1].bubbleType).to.equal("SystemMessageBubble")
			end)
			...
			it("should create two cards when presented a link with text if the link is second", function()
				...
				expect(chat.bubbles[2].placeId).to.equal("1818")
			end)
			...
			it("should optionally accept the final / when parsing a roblox link", function()
				...
				expect(#chat2.bubbles).to.equal(1)
				expect(chat2.bubbles[1].bubbleType).to.equal("AssetCard")
				expect(chat2.bubbles[1].placeId).to.equal("1337")
			end)
			...
			it("should handle text in between two links", function()
				...
				expect(chat.bubbles[3].placeId).to.equal("1337")
			end)
		end)
	end)
end