...
local request = require(LuaApp.Http.request)
local ApiFetchGameDetails = require(LuaApp.Thunks.ApiFetchGameDetails)
local RetrievalStatus = require(Modules.LuaApp.Enum.RetrievalStatus)
local openGameDetailsPage = require(LuaChat.Thunks.openGameDetailsPage)
...
local FFlagLuaChatNavigateToLuaGameDetails = FlagSettings.isLuaGameDetailsEnabled()
...
local ASSET_CARD_HORIZONTAL_MARGIN_COMPACT = 108
local ASSET_CARD_HORIZONTAL_MARGIN_WIDE = 224
...
function AssetCard.new(appState, message, placeId, universeId)
	local self = {}
	...
	self.placeId = placeId or nil
	self.universeId = universeId or nil
...
function AssetCard:Resize()
	...
	local bubbleSizeOffsetY = formFactor == FormFactor.COMPACT and ASSET_CARD_HORIZONTAL_MARGIN_COMPACT
		or ASSET_CARD_HORIZONTAL_MARGIN_WIDE
...
function AssetCard:Update(newState)
	if FFlagLuaChatGameLinkRender then
		if self.placeId then
			self:_fetchAndUpdatePlaceIdInfo(newState)
		elseif self.universeId then
			self:_fetchAndUpdateUniverseIdInfo(newState)
		end
	else
		local placeInfo = newState.ChatAppReducer.PlaceInfos[self.placeId]
	...
	self.cardBodyClick = self.Content.Activated:Connect(function()
		self:ReportAnEvent(LINK_CARD_CLICKED_EVENT, TOUCH_CONTEXT)
		if self.placeInfo then
			if FFlagLuaChatNavigateToLuaGameDetails then
				self.appState.store:dispatch(openGameDetailsPage(self.universeId))
			else
				GuiService:BroadcastNotification(self.placeId,
					NotificationType.VIEW_GAME_DETAILS)
			end
		end
	end)

	self.detailsButtonClick = self.actionButton.Activated:Connect(function()
		...
		if self.placeInfo then
			if self.placeInfo.isPlayable then
				...
				local gameParams = GameParams.fromPlaceId(self.placeId)
...
function AssetCard:_fetchAndUpdatePlaceIdInfo(newState)
	local placeInfo = newState.ChatAppReducer.PlaceInfos[self.placeId]
	if placeInfo == nil then
		self:ShowLoadingIndicator(true)
		self.appState.store:dispatch(GetMultiplePlaceInfos({self.placeId}))
...
function AssetCard:_fetchAndUpdateUniverseIdInfo(newState)
	local universeInfo = newState.GameDetails[self.universeId]
	if universeInfo then
		self.placeId = universeInfo.rootPlaceId
		self:_fetchAndUpdatePlaceIdInfo(newState)
	else
		self:ShowLoadingIndicator(true)
		if ApiFetchGameDetails.GetFetchingStatus(newState, self.universeId) == RetrievalStatus.NotStarted then
			self.appState.store:dispatch(ApiFetchGameDetails.Fetch(request, {self.universeId}))
		end
	end
end
