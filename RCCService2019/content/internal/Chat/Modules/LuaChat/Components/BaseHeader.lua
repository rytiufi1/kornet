...
local GetImageSetData = require(LuaApp.GetImageSetData)
local GetImageSetPathPattern = require(LuaApp.GetImageSetPathPattern)
...
if UseNewAppStyle then
	PLATFORM_SPECIFIC_CONSTANTS = {
		[Enum.Platform.Android] = {
			BACK_BUTTON_ASSET_ID = "rbxasset://textures/ui/LuaChatV2/navigation_pushBack.png",
		},
...
local HOME_LOGO = "LuaApp/icons/ic-roblox-close"
		...
		return Roact.createElement(RoactRodux.StoreProvider, {
			...
			serviceProvider = Roact.createElement(RoactServices.ServiceProvider, {
				services = {
					...
					[RoactNetworking] = self.appState.request,
				}
			}, {
				newGameDrawer
			})
...
if FFlagLuaChatHeaderEnableHomeButton then
	function BaseHeader:GetNewBackButton(buttonType)
		local backButton
		if buttonType == BaseHeader.ButtonType.Modal then
			backButton = PaddedImageButton.new(self.appState, "Close", "rbxasset://textures/ui/LuaChat/icons/ic-close-gray2.png")
		elseif buttonType == BaseHeader.ButtonType.Popup then
			backButton = PaddedImageButton.new(self.appState, "Close", "rbxasset://textures/ui/LuaChat/icons/ic-close-white.png")
		elseif buttonType == BaseHeader.ButtonType.Logo then
			local homeLogo
			if FFlagLuaAppHomeIconPolicy and self.appState.AppPolicy then
				homeLogo = self.appState.AppPolicy.getHomeIcon() or HOME_LOGO
			else
				homeLogo = HOME_LOGO
			end
			local imageData = GetImageSetData(GuiService:GetResolutionScale())[homeLogo]
			backButton = PaddedImageButton.new(self.appState, "LogoHome", GetImageSetPathPattern(imageData.ImageSet))
			backButton.rbx:FindFirstChildOfClass("ImageLabel").ImageRectOffset = imageData.ImageRectOffset
			backButton.rbx:FindFirstChildOfClass("ImageLabel").ImageRectSize = imageData.ImageRectSize
		else
		...
		--Get theme data if flag is on
		if UseNewAppStyle then
			local theme = GetCurrentTheme()

			backButton.rbx.ImageLabel.ImageColor3 = theme.SystemPrimaryButton.Color
			backButton.rbx.ImageLabel.ImageTransparency = theme.SystemPrimaryButton.Transparency
		end

		return backButton
	end

	function BaseHeader:getButtonTypeFromDialogType(dialogType)
		if dialogType == DialogInfo.DialogType.Modal then
			return BaseHeader.ButtonType.Modal
		elseif dialogType == DialogInfo.DialogType.Popup then
			return BaseHeader.ButtonType.Popup
		end
