...
local SendTextMessage = require(LuaChat.Thunks.SendTextMessage)
...
local FFlagLuaChatOnlyMarkUnreadWhenChatIsFocused = settings():GetFFlag("LuaChatOnlyMarkUnreadWhenChatIsFocused")
...
function ConversationView.new(appState)
	...
	if appState.store:getState().FormFactor == FormFactor.COMPACT then
	...
	if isFeatureEnabled(self.appState, AppFeature.ChatConversationHeaderGroupDetails) then
	...
	local icebreaker = Icebreaker.new(self.appState)
	...
	icebreaker.SendButtonPressed:connect(function(text)
		...
		self.appState.store:dispatch(SendTextMessage(self.conversationId,
			text, Constants.Decorators.ICEBREAKER))
	end)
	...
	chatInputBar.SendButtonPressed:connect(function(text)
		text = sendPreprocess(text)
		appState.store:dispatch(SendTextMessage(self.conversationId, text))
	end)
...
function ConversationView:Update(state)
	...
	if FFlagLuaChatNavigateToLuaGameDetails then
		...
		for _, section in ipairs(currentRoute) do
			if section.name ~= AppPage.Chat then
				self.chatInputBar.textBox:ReleaseFocus()
			end
		end
	end
	...
	if currentConversationId and currentConversationId ~= self.conversationId then
		...
		self.readAllMessagesConnection = messageList.ReadAllMessages:connect(function()
			if FFlagLuaChatOnlyMarkUnreadWhenChatIsFocused then
				if not self.appState.store:getState().ChatAppReducer.ToggleChatPaused then
					self.appState.store:dispatch(ConversationActions.MarkConversationAsRead(self.conversationId))
				end
			else
				self.appState.store:dispatch(ConversationActions.MarkConversationAsRead(self.conversationId))
		...
		if self.messageList.isTouchingBottom then
			if FFlagLuaChatOnlyMarkUnreadWhenChatIsFocused then
				if not state.ChatAppReducer.ToggleChatPaused then
					self.appState.store:dispatch(ConversationActions.MarkConversationAsRead(self.conversationId))
				end
			else
				self.appState.store:dispatch(ConversationActions.MarkConversationAsRead(self.conversationId))
