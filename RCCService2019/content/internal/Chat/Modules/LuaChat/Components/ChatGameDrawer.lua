...
local ChatConstants = require(LuaChat.Constants)
...
local PINNED_TEXT_FONT = ChatConstants.Font.STANDARD
...
if FFlagLuaChatNavigateToLuaGameDetails then
	function ChatGameDrawer:ViewGameDetails(universeId)
		local playTogetherViewDetails = self.props.playTogetherViewDetails
		playTogetherViewDetails(universeId)
	end
else
...
function ChatGameDrawer:GameStart(game)
	...
	if #friends == 0 then
		...
		self:GamePlayConversationId(placeId, conversationId)
	else
...
function ChatGameDrawer:GamePlayConversationId(placeId, conversationId)
	-- Report player start game via play together:
	self:ReportGamePlayIntent(placeId)
	self:ReportPlayerPlayGame(placeId)

	-- Start a game:
	local gameParams = GameParams.fromConversationId(placeId, conversationId)
	local payload = HttpService:JSONEncode(gameParams)
	GuiService:BroadcastNotification(payload, GuiService:GetNotificationTypeList().LAUNCH_GAME)
end
...
function ChatGameDrawer:render()
	...
	for _, game in ipairs(gameInfo.games) do
		if game.pinned then
			...
			local onViewDetails
			if FFlagLuaChatNavigateToLuaGameDetails then
				onViewDetails = function(universeId)
					self:ViewGameDetails(universeId)
				end
			else
	...
	function(dispatch)
		if FFlagLuaChatNavigateToLuaGameDetails then
			return {
				...
				playTogetherViewDetails = function(universeId)
