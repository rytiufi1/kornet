...
local ActionButton = require(LuaChat.Components.ChatGameCardComponents.ActionButton)
...
local FFlagLuaChatJoinFriendGameButtonResizeFix = settings():GetFFlag("LuaChatJoinFriendGameButtonResizeFix")
...
function ChatGameCard:render()
	...
	if isPlayable then
		...
		if (FFlagLuaChatJoinFriendGameButtonResizeFix) then
			buttonWidth = Text.GetTextWidth(actionText, GAME_TEXT_FONT, GAME_TEXT_SIZE)
		end
	...
	RowLayoutOrder = RowLayoutOrder + 1
	...
	if isPlayable then
		if FFlagLuaChatJoinFriendGameButtonResizeFix then
			cardItems["ActionButton"] = Roact.createElement(ActionButton, {
				textPadding = 5,
				onActivated = onGameStart,
				layoutOrder = RowLayoutOrder,
				text = actionText,
			})
		else
			...
			cardItems["ActionButton"] = Roact.createElement("ImageButton", {
				...
				SliceCenter = Rect.new(3,3,4,4),
				[Roact.Event.Activated] = onGameStart,
			}, {
