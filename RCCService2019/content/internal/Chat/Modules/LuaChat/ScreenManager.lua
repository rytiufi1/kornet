...
local FFlagLuaChatScreenManagerResumeAndPauseAfterNavigation =
	settings():GetFFlag("LuaChatScreenManagerResumeAndPauseAfterNavigation")
...
function ScreenManager:EnableAllViews()
	...
	if FFlagLuaChatScreenManagerResumeAndPauseAfterNavigation then
		local current = self:GetCurrentView()
		if current then
			current:Resume()
		end
