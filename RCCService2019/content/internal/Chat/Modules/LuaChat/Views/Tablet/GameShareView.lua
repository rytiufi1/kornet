...
function GameShareView.new(appState, route)
	...
	local dividerHeight = Constants.GameShareView.WIDE_HORIZONTAL_DIVIDER_HEIGHT
	local viewWidth = Constants.GameShareView.WIDE_VIEW_WIDTH
	...
	self.setTabBarVisible = function(isVisible)
		if not IsLuaBottomBarEnabled then
			self.appState.store:dispatch(SetTabBarVisible(isVisible))
		else
			self.appState.store:dispatch(SetTabBarVisibleFromChat(isVisible))
		end
	end
