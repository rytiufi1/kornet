...
local FFlagLuaChatMarkUnreadAndFlush369 = settings():GetFFlag("LuaChatMarkUnreadAndFlush369")
...
function ConversationActions.MarkConversationAsRead(conversationId)
	return function(store)
		...
		if lastUnreadMessage or (conversation.hasUnreadMessages and count == 0) then
			...
			if FFlagLuaChatMarkUnreadAndFlush369 then
				--[[
					Since Rodux is asynchronous and the store will batch and
					update in the next frame, we want to flush the store now.
					This will prevent the UI from displaying that the message
					is unread and then read a frame later. Without a flush,
					you see a weird visual blink.
				]]
				store:flush()
			end
