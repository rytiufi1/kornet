...
local FFlagGetPlaceThumbnailUseCdnUrl = settings():GetFFlag("LuaChatGetPlaceThumbnailUseCdnUrl")
			...
			while (retryCount > 0) do
				...
				if status ~= WebApi.Status.OK then
					...
					if placeThumbnailData.final == true then
						if FFlagGetPlaceThumbnailUseCdnUrl then
							thumbnail = placeThumbnailData.cdnUrl
						else
