...
function RobloxEventReceiver:init(store)
	...
	local function onPrivacyNotifications(eventData)
		...
		if detailType == "ChatDisabled" then
			store:dispatch(FetchChatSettingsCompleted({ chatEnabled = false }))
		elseif detailType == "ChatEnabled" then
			store:dispatch(FetchChatSettingsCompleted({ chatEnabled = true }))
		end
	end
		...
		if connectionHubName == "signalR" then
			...
			if connectionState == Enum.ConnectionState.Connected then
				if seqNum ~= lastSeqNum then
					local function refreshConvos(chatEnabled)
						if chatEnabled then
					...
					if store:getState().ChatAppReducer.AppLoaded then
						store:dispatch(FetchChatSettings(function(settings)
							refreshConvos(settings.chatEnabled)
						end))
					end
