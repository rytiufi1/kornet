...
local function playerInputReducer(state, action)
	...
	if action.type == "SetPlayerInput" then
		local newInput = {}
		newInput[action.playerInput] = action.value


		return Cryo.Dictionary.join(state, newInput)
	end
...
local function playerReducer(state, action)
	...
	elseif action.type == "SetPlayerRotation" then
		return Cryo.Dictionary.join(state, { angle = action.value, })
	end
...
function Camera:didMount()
	Workspace.CurrentCamera = self.cameraRef.current
end

-- A version of the camera that's tied to the player's position.
local PlayerCamera = RoactRodux.UNSTABLE_connect2(
...
Asteroids = RoactRodux.UNSTABLE_connect2(
	function(state, props)
...
local PlayerShip = RoactRodux.UNSTABLE_connect2(
	function(state, props)
		return {
...
local KeyboardInputConnector = RoactRodux.UNSTABLE_connect2(
	nil,
	function(dispatch)
		return {
...
function RenderSteppedConnector:render()
	local playerInputs = self.props.playerInputs

	local function onStepped(delta)
		self.props.onStepped(delta, playerInputs)
	end
...
RenderSteppedConnector = RoactRodux.UNSTABLE_connect2(
	function(state, props)
		return {
	...
	function(dispatch)
		return {
			onStepped = function(delta, playerInputs)
				dispatch({
					type = "GameStep",
					delta = delta,
					inputs = playerInputs,
...
PauseRoute = RoactRodux.UNSTABLE_connect2(
	function(state, props)
		return {
			paused = state.paused
		}
...
local function main()
	...
	Roact.mount(app, Workspace, "Roact-asteroids")
	Roact.mount(ui, CoreGui, "Roact-asteroids-ui")
end
