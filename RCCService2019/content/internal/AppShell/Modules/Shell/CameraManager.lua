...
local eternalRenderStepOn = false
...
local function adjustAvatarEditorLighting()
	Blur.Enabled = false
	ColorCorrection.Contrast = 0.35
	ColorCorrection.Saturation = 0.175
...
local function stopEternalRenderStep()
	eternalRenderStepOn = false
...
local function transitionToCameraAnimator(cameraAnimator, transitionDuration, targetBrightness, targetBlurEnabled, targetBloomEnabled)
	...
	function()
		...
		function()
			-- Check if the avatar editor has been enabled before this tween finished.
			if not eternalRenderStepOn then
				adjustAvatarEditorLighting()
			end
		end)
	end)
end
...
local switchToAvatarEditorAfterInitialization = false
...
function CameraManager:CameraMoveToAsync()
	...
	if switchToAvatarEditorAfterInitialization then
...
function CameraManager:SwitchToFlyThrough()
	switchToAvatarEditorAfterInitialization = false
	if not flythroughAnimator then
		return
	end
...
function CameraManager:SwitchToAvatarEditor()
	-- In case the Avatar Editor is accessed before the camera gets initialized.
	if not flythroughAnimator or not avatarEditorCameraAnimator then
		switchToAvatarEditorAfterInitialization = true
		return
	end

	stopEternalRenderStep()

	flythroughAnimator:Stop()
	ZoneManager:SetZone("AvatarEditor")
	avatarEditorCameraAnimator:Reset()

	-- Adjust lighting now that the Avatar Editor handles its own camera.
	adjustAvatarEditorLighting()
...
function CameraManager:UpdateAvatarEditorCamera(newCFrame)
	-- It's possible for the Roact Avatar Editor to able to update the camera faster than this can be created.
	if not avatarEditorCameraAnimator then
		return
	end
