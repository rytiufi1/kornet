...
do
	...
	local function getDelimiterByLocale()
		...
		if delimiterByLocale[locale] then
			return delimiterByLocale[locale]
		end

		return delimiterByLocale["en-us"]
	end

	function Util.FormatNumberString(value)
		local delimiter = getDelimiterByLocale()
		...
		local substitutions
		repeat
			stringValue, substitutions = string.gsub(stringValue, "^(-?%d+)(%d%d%d)", delimiterReplace)
