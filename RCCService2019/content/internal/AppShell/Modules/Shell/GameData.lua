...
function GameData:UpdateGameData(game)
		...
		local function RefreshGameDetailsAsync(gameData)
			...
			if not result then
				...
				if startCount == UserChangedCount then
					...
					gameData.Name = gameData.Name or gameDetail.name
					...
					gameData.CreatorUserId = gameDetail.creatorUserId
...
function GameData:ClearSharedGameData()
	if PlatformService then
		local sharedData = PlatformService:ReadSharedData()
		sharedData.games = nil
		PlatformService:WriteSharedData(sharedData)
	end
end

function GameData:WriteSharedGameData(gamesList)
	if PlatformService then
		local sharedData = PlatformService:ReadSharedData()
		sharedData.games = sharedData.games or {}
		for _,placeId in pairs(gamesList) do
			sharedData.games[tostring(placeId)] = gameCachedData[placeId]
		end
		PlatformService:WriteSharedData(sharedData)
	end
end
