...
local XboxShowCuratedSorts = settings():GetFFlag("XboxShowCuratedSorts")
...
do
	-- TODO: We need to make sorts more dynamic. Right now get get all sorts and filter
	-- for Home and Games page. We should be getting sorts for each page and filter based
	-- on that result, rather than a hard coded filter

	--Sorts to include
	if not XboxShowCuratedSorts then
	...
	local function ResetCacheData()
		...
		if FFlagXboxPlayNextGame then
			if PlatformService then
				local sharedData = PlatformService:ReadSharedData()
				sharedData.sorts = nil
				PlatformService:WriteSharedData(sharedData)
			end
		end
	end
	...
	local function _createSort(id, sortName, httpFunc, userId, gameSetTargetId, useTokens, sortDisplayName, timeFilter)
		...
		--If the sort is related to user, we store the userId (MyFavorite, MyRecent and MyPlaces)
		--Note: Only MyPlaces really needs userId info, the other two can fecth user info on the web side
		if userId then
			sort.userId = userId
		end

		--NOTE: Remove when game lists no longer use image ids
		...
		elseif sort.Id == SortsData.DefaultSortId.MyFavorite and sort.GameSetTargetId == 0 then
		...
		elseif sort.Id == SortsData.DefaultSortId.MyRecent and sort.GameSetTargetId == 0 then
			...
			EventHub:addEventListener(sort.eventString, sort.objectIDString,
				function(success, placeId)
					...
					--Write sort cache to shared data
					if FFlagXboxPlayNextGame then
						SortsData:UpdateSharedSortData(sort.Id, cachedGamesData)
					end
					isLoading = false
				end)
		end
		...
		function sort:UpdateCachedGamesData(data, pagingIndex, scrollLeft)
			...
			for i = indexFrom, indexTo, step do
				...
				cachedGamesData[i] = {}
				cachedGamesData[i].PlaceId = game.placeId
...
--				Utility.DebugLog("SortsData: has cached data", pageIndex, hasPrevPage)
			...
			--Write sort cache to shared data
			if FFlagXboxPlayNextGame then
				if sort.Id == SortsData.DefaultSortId.MyRecent or sort.Id == SortsData.DefaultSortId.Featured then
					SortsData:UpdateSharedSortData(sort.Id, cachedGamesData)
				end
			end
			isLoading = false
			...
			if not hasCached then
				...
				if data then
					...
					for i = 1, #listData do
						...
						cachedGamesData[startIndex + i].PlaceId = game.placeId
						cachedGamesData[startIndex + i].IsSponsored = listData[i].isSponsored
						cachedGamesData[startIndex + i].NativeAdData = listData[i].nativeAdData
						cachedGamesData[startIndex + i].LastAccessTime = tick()
						currGameCount = currGameCount + 1
						if sort.Id == SortsData.DefaultSortId.MyRecent or sort.Id == SortsData.DefaultSortId.Featured then
							table.insert(sharedGamesList, game.placeId)
						end
					end
					...
					ClearCaching()
					if FFlagXboxPlayNextGame then
						GameData:WriteSharedGameData(sharedGamesList)
					end
				end
			end
			for index = startIndex + 1, startIndex + pageSize do
				if cachedGamesData[index] then
					...
					if cachedGamesData[index].HasData then
						local gameData = {
							placeId = cachedGamesData[index].PlaceId,
							isSponsored = cachedGamesData[index].IsSponsored,
							nativeAdData = cachedGamesData[index].NativeAdData,
						}
						table.insert(gamesData, gameData)
					else
						break
					end
				end
...
--					Utility.DebugLog("sortsData:InitSortAsync: pageIndex", i, "hasPage", page and #page)
	...
	--NOTE: Remove when game list no longer use image ids
	...
	--NOTE: Change name to createSort when all game lists use tokens
	...
	local function GetSortsAsync(userId)
		...
		--NOTE: This sort will still require the use of image id to get Thumbnails.
	...
	function SortsData:GetUserFavorites()
		CheckCachedData()
		return GetSortFromSortId(SortsData.DefaultSortId.MyFavorite)
	end

	function SortsData:GetUserRecent()
		CheckCachedData()
		return GetSortFromSortId(SortsData.DefaultSortId.MyRecent)
	end
	...
	function SortsData:UpdateSharedSortData(sortId, gamesData)
		if PlatformService then
			local sharedData = PlatformService:ReadSharedData()
			sharedData.sorts = sharedData.sorts or {}
			sharedData.sorts[tostring(sortId)] = {
				orderedSort = gamesData
			}
			PlatformService:WriteSharedData(sharedData)
		end
	end
