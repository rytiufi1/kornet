...
function ScreenManager:CloseCurrent(id)
	...
	-- ignore if the calling screen is not currently the top. This works by using getState
	-- because state is updated immediately while changed events fire once a frame
	if id and frontScreen.id ~= tostring(id) then
		return
	end
