...
local function CreateAppHub()
	...
	local AppTabDock = AppTabDockModule(
		...
		UDim2.new(0,0,0,0))
	local appHubCns = {}
	local selectedTabChangedCn = nil
	...
	local function onOpenAvatarEditor(actionName, inputState, inputObject)
		...
		elseif inputState == Enum.UserInputState.End and seenYButtonPressed then
			EventHub:dispatchEvent(EventHub.Notifications["OpenAvatarEditorScreen"])
	...
	function this:Show()
		...
		selectedTabChangedCn = AppTabDock.SelectedTabChanged:connect(onSelectedTabChanged)
	end
	...
	function this:Hide()
		...
		EventHub:removeEventListener(EventHub.Notifications["UnlockedUGC"], 'AppHubUnlockedUGC')
		Utility.DisconnectEvent(selectedTabChangedCn)
